...
<?php
$title = "feedback | SLGTI";
include_once("../config.php");
include_once("../head.php");
include_once("../menu.php");
$eid =$s_staffname = $s_dept = $s_module = $s_edate =null;
if(isset($_GET['id'])){
    $get_survey_id = intval($_GET['id']); // Convert to integer for safety
    $sql = "SELECT `feedback_survey`.`survey_id` AS `survey_id`,
            ...
            INNER JOIN `course` ON `feedback_survey`.`course_id` = `course`.`course_id`
            INNER JOIN `module` ON `feedback_survey`.`module_id` = `module`.`module_id`
            INNER JOIN `staff` ON `feedback_survey`.`staff_id` = `staff`.`staff_id`
            WHERE `feedback_survey`.`survey_id` = ?";
    
    $stmt = mysqli_prepare($con, $sql);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "i", $get_survey_id);
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            echo "<div class='alert alert-danger'>Error: No matching record found.</div>";
        }
    ...
    echo "<div class='alert alert-warning'>No survey ID provided.</div>";
}
...
$survey_id=$eid; 
    ...
    if (isset($_POST['add'])) {
        ...
        !empty($_POST['feedback_q17'])&&!empty($_POST['feedback_q18'])&&!empty($_POST['feedback_commond'])){
            ...
            // Prepare the first statement for feedback insertion
            $sql = "INSERT INTO `feedback` (`feedback_q1`, `feedback_q2`, `feedback_q3`, `feedback_q4`, `feedback_q5`, `feedback_q6`, `feedback_q7`, `feedback_q8`, `feedback_q9`, `feedback_q10`, `feedback_q11`, `feedback_q12`, `feedback_q13`, `feedback_q14`, `feedback_q15`, `feedback_q16`, `feedback_q17`, `feedback_q18`, `feedback_commond`, `survey_id`) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            $stmt = mysqli_prepare($con, $sql);
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssssssssssssssssss", 
                    $feedback_q1, $feedback_q2, $feedback_q3, $feedback_q4, $feedback_q5, 
                    $feedback_q6, $feedback_q7, $feedback_q8, $feedback_q9, $feedback_q10,
                    $feedback_q11, $feedback_q12, $feedback_q13, $feedback_q14, $feedback_q15,
                    $feedback_q16, $feedback_q17, $feedback_q18, $feedback_commond, $survey_id
                );
                
                if(mysqli_stmt_execute($stmt)) {
                    echo "<h4 class='text-success'>New record created successfully</h4>";
                    
                    // Prepare the second statement for feedback_done
                    $uname = $_SESSION["user_name"];
                    $sql2 = "INSERT INTO feedback_done (survey_id, student_id, s_date, s_time) VALUES (?, ?, CURDATE(), CURTIME())";
                    $stmt2 = mysqli_prepare($con, $sql2);
                    
                    if ($stmt2) {
                        mysqli_stmt_bind_param($stmt2, "is", $survey_id, $uname);
                        if(!mysqli_stmt_execute($stmt2)) {
                            echo "<div class='alert alert-warning'>Feedback recorded, but there was an issue updating the completion status.</div>";
                        }
