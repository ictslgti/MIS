...
<div class="col-2"></div>
 ...
 <div class="col-1"> 
 <button type="submit" class="btn btn-danger" onclick="location.href='donationInfo.php'" ><i class="fas fa-backspace"></i>&nbsp;&nbsp;</button>
...
if(isset($_POST['Add']))
{
   ...
   if (!empty($_POST['id']) && !empty($_POST['date'])&& !empty($_POST['programme']))
   {
    
    //    $id = $_POST['id'];
       $date = $_POST['date'];
       $programme = $_POST['programme'];
       $sql = "INSERT INTO `donation` (`donation_id`,`date`,`programme`)VALUES(null,'$date','$programme')";
...
if(isset($_POST['Edit'])){
    if (!empty($_POST['id']) && !empty($_POST['date'])&& !empty($_POST['programme'])&& !empty($_GET['edit']))
    {
        $id = $_POST['id'];
        $date = $_POST['date'];
        $programme = $_POST['programme'];
...
<input class="form-control form-control-sm ml-3 w-75 rounded-pill" type="search" placeholder="Search_id" aria-label="Search" id="edit"> 
